...
export class SheetTransactionHistory extends Component {
    ...
    async fetchAndCache() {
        ...
        for (const canisterId of this.tokens) {
            if (this.app.cache.storage.get({ id: `history:${this.wallet.principal}:${canisterId}`, overdue: ONE_MINUTE * 10 })) {
                // Get token
                const token = this.wallet.tokens.get(canisterId);
                if (token) {
                    // Refresh token info in weekly basis
                    if (!this.app.isICP(canisterId) && this.app.cache.storage.get({ id: `token:${canisterId}`, overdue: ONE_WEEK })) {
                        let changed = false;
